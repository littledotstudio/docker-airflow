...
print("def functions")
dogleash_search = ['Pet Supplies', 'Pet Storage', 'pet leash holder', 'harness', 'collar', 'custom leash holder', 'pet gift', 'dog leash holder', 'dog gift', 'personal gift', 'leash', 'pet storage', 'pet accessories', 'dog accessories']

for i in range(0, len(dogleash_search)):
    df = get_query_df(dogleash_search[i])
    print("got dataframe for tag, ", i)
    df['date'] = today
    df['rank'] = np.arange(len(df))
    df['tag'] = dogleash_search[i]
    # Connecting with `gspread` here
    sheet = client.open("etsydata").get_worksheet(0)
...
coasters_search = ['Drinkware', 'Coasters', 'wife', 'boyfriend', 'gift', 'minimal', 'craft', 'laser cut', 'bees', 'honeycomb', 'acrylic coaster']

for i in range(0, len(coasters_search)):
    df = get_query_df(coasters_search[i])
    print("got dataframe for tag, ", i)
    df['date'] = today
    df['rank'] = np.arange(len(df))
    df['tag'] = coasters_search[i]
    # Connecting with `gspread` here
    sheet = client.open("etsydata").get_worksheet(1)
    existing = gd.get_as_dataframe(sheet)
    n = existing.list_id.count()
    set_with_dataframe(sheet, df, row=n+2, include_column_header=False)
    print("set tag: ", i)
